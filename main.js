...
footer.innerHTML = `&copy; ${new Date().getFullYear()} <a href="https://github.com/carsnl" target="_blank">Lai Carson</a>`;
...
searchQuery.setAttribute("placeholder", `${getPlaceholder()}`);
...
async function fetchNews(isNewSearch) {
  const fetchMoreBtnContainer = document.querySelector(
...
function getPlaceholder() {
  return searchPlaceholder[
    Math.floor(Math.random() * searchPlaceholder.length)
  ];
}
